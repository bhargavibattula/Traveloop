...
import { Sparkles, Send, Loader2, Bot, Wand2, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div className="planner-root">
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .header h1 { font-family: 'Cormorant Garamond', serif; font-size: 56px; font-weight: 700; line-height: 1.1; margin-bottom: 12px; }

        .badge {
          display: inline-flex; align-items: center; gap: 8px; background: var(--bg-peach); color: var(--primary);
          padding: 6px 16px; border-radius: 100px; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 1.5px;
        }

        .feed { flex: 1; overflow-y: auto; display: flex; flex-direction: column; gap: 32px; padding-right: 12px; margin-bottom: 40px; }
        .bubble { max-width: 80%; padding: 24px 32px; border-radius: 32px; font-size: 16px; line-height: 1.6; }
        .bubble.ai { background: white; color: var(--text-dark); border-bottom-left-radius: 4px; box-shadow: 0 4px 15px rgba(0,0,0,0.02); }
        .bubble.user { background: var(--primary); color: white; align-self: flex-end; border-bottom-right-radius: 4px; box-shadow: 0 10px 30px rgba(255, 107, 107, 0.15); }

        .input-form {
          position: relative; background: white; border-radius: 32px; padding: 8px; 
          box-shadow: 0 15px 40px rgba(0,0,0,0.05); border: 1px solid var(--border);
          display: flex; align-items: center; gap: 12px;
        }

        .input-box {
          flex: 1; border: none; outline: none; padding: 16px 24px; font-family: inherit; font-size: 17px;
          resize: none; background: transparent; color: var(--text-dark); min-height: 56px; line-height: 1.4;
        }
        ...
        .back-btn {
          ...
          font-weight: 700; text-decoration: none; font-size: 14px; cursor: pointer; border: none; background: none;
        }
        .back-btn:hover { color: var(--primary); }

        @media (max-width: 768px) {
          .chat-container { padding: 80px 20px 40px; }
          ...
          <div className="top-bar">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              <div className="hidden sm:block" style={{ textAlign: 'right' }}>
                ...
                <div style={{ fontWeight: 700, fontSize: '14px' }}>Travel Engine</div>
              </div>
              <div style={{ width: 44, height: 44, background: 'var(--bg-peach)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <AnimatePresence>
                ...
                {loading && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bubble ai" style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    ...
                    <span style={{ fontWeight: 700 }}>Optimizing logistics...</span>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
